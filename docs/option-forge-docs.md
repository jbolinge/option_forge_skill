...
This page documents the OptionForge Lua API. For the main app, see [OptionForge](/). For parameter sweeps, see [Grid Search](/grid-search).

Need help with OptionForge? [ [option-forge GPT]](https://chatgpt.com/g/g-69a3380046e08191bf9358cba16085cb-option-forge) is a custom GPT on OpenAI that can answer questions about the Lua API and help with OptionForge strategy ideas, implementation, and debugging.

## Table of Contents

- [General](#general)
- [Portfolio Management](#portfolio)
- [Trade Operations](#trades)
- [Leg Management](#legs)
- [Leg Properties](#leg-properties)
- [Option Selectors](#selectors)
- [Moving Averages](#moving-averages)
- [Global Variables](#globals)
- [CSV User Data](#csv-user-data)
- [Plotting](#plotting)
- [Complete Examples](#examples)

## General

OptionForge uses [Luau](https://luau.org/library). See the [Lua cheatsheet](https://devhints.io/lua) if you need a refresher.

Your script runs on _every_ tick. Gate entries, but usually manage exits and adjustments every tick.

_The quick example below covers most common patterns._
...
#### Quick Example (See [Complete Examples](#examples) for more)
...
[new_trade](#portfolio-new-trade) [n_open_trades](#portfolio-n-open-trades) [trades](#portfolio-trades) [history](#portfolio-history) [pnl](#portfolio-pnl) [value](#portfolio-value) [delta/theta](#portfolio-delta) [iv](#portfolio-iv) [last_trade](#portfolio-last-trade) [count](#portfolio-count) [trade](#portfolio-trade)
...
[pnl](#trade-pnl) [mid](#trade-mid) [id](#trade-id) [dte](#trade-dte) [dit](#trade-dit) [delta/gamma/vega/theta](#trade-greeks) [iv](#trade-iv) [close](#trade-close) [cash](#trade-cash) [erase](#trade-erase) [add_leg](#trade-add-leg) [close_leg](#trade-close-leg) [adjust](#trade-adjust) [leg](#trade-leg) [risk_graph](#trade-risk-graph) [export](#trade-export)
...
[Delta](#selector-delta) [TradeDelta](#selector-tradedelta) [Strike](#selector-strike) [Mid](#selector-mid) [Theta](#selector-theta) [Vega](#selector-vega) [Gamma](#selector-gamma)
...
Table populated from [CSV user data](#csv-user-data).
...
- `start_date` (string or nil): "YYYY-MM-DD" (default nil = first archive day)
- `end_date` (string or nil): "YYYY-MM-DD" (default nil = last archive day)
...
- `archive` (string or nil): named archive (for example "SPX", "VIX", "NDX", or "RUT"), requires server-side archive registry configuration
- `csv` (string or nil): public CSV URL for [user data](#csv-user-data)
- `tick_interval` (string): "day", "1-day", "hour", "1-hour", or minutes ("1-minute", "5-minute", "15-minute", "30-minute"). The resolution available depends on the archive. Defaults to "day".
- `tick_time` (number or "HH:MM"): daily sample time in HHMM (default 1000). Use `"10:30"` to specify a time string. Use `9999` to pick a random available time per run. If the chosen time is not present anywhere in the archive, the run errors. If missing on a given day, the nearest tick is used and a warning is logged.
...
Titles starting with "**main_**" plot on the main chart's second y-axis. Only scatter plots are supported there. Other plots go to User Plots.
...
- opts (table): Options for the plot. "bins" (number), "color" (string), "symbol" (string), "date" (string), "hovertext" (string), "trace" (string)

See [Plotly Scatter](https://plotly.com/javascript/reference/scatter/#scatter-marker-symbol) for possible symbols.
